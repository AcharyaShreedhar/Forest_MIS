...
export function* fetchgairakasthabanpaidawarbikribitaranRequest(api, action) {
  const { payload } = action
  const payloaddata = isNil(payload) ? action : payload
  const response = yield api.postGairakasthaBanpaidawarBikribitaran(payloaddata)
...
export function* fetchbudgetmonthlybibaranRequest(api, action) {
  const { payload } = action
  const payloaddata = isNil(payload) ? action : payload
  const response = yield api.postBudgetMonthlyBibaran(payloaddata)
  if (response.ok) {
    yield put(ReportActions.fetchbudgetmonthlybibaranSuccess(response.data))
  } else {
    yield put(ReportActions.fetchbudgetmonthlybibaranFailure())
  }
}
