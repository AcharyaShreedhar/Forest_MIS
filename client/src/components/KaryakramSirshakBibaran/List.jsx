import React, { Fragment } from 'react'
import { englishToNepaliNumber } from 'nepali-number'
import { PropTypes } from 'prop-types'
import { isNil } from 'ramda'
import { Table } from 'react-bootstrap'
import { Button, EditDropdown, Pagination } from '../../components'

function List(props) {
  const {
    buttonName,
    headings,
    data,
    title,
    pageCount,
    onAdd,
    onSelect,
    onPageClick,
    pers,
    per,
    onPer,
    role,
    forcePage,
  } = props
  return (
    <Fragment>
      <div className="card">
        <div className="button">
          <Button type="low" size="small" name={buttonName} onClick={onAdd} />
        </div>
        <div className="titlebar">{title} </div>
        <Table responsive striped bordered hover id="karyakramsirshak">
          <thead>
            <tr>
              <th>क्र.स.</th>
              {headings.map((heading, index) => (
                <th key={index}>{heading}</th>
              ))}
              <th />
            </tr>
          </thead>
          <tbody>
            {isNil(data) ? (
              <p>No data Available !!!</p>
            ) : (
              data.map((karyakramsirshak, index) => (
                <tr key={`${karyakramsirshak.karyakram_sirshak_id}-${index}`}>
                  <td>{englishToNepaliNumber(index + 1)}</td>
                  <td>{karyakramsirshak.karyakram_name}</td>
                  <td>{karyakramsirshak.karyakram_sirshak_no}</td>
                  <td>{karyakramsirshak.created_by}</td>
                  <td>
                    <div className="edit">
                      <EditDropdown
                        options={role < 3 ? ['Edit'] : ['Edit', 'Delete']}
                        onChange={(e) =>
                          onSelect(e, karyakramsirshak, 'karyakramsirshak')
                        }
                      />
                    </div>
                  </td>
                </tr>
              ))
            )}
          </tbody>
        </Table>
        <Pagination
          per={per}
          forcePage={forcePage}
          pers={pers}
          onPer={onPer}
          onPageClick={onPageClick}
          pageCount={pageCount}
          type="karyakramsirshak"
        />
      </div>
    </Fragment>
  )
}

List.propTypes = {
  data: PropTypes.array,
  onSelect: PropTypes.func,
}

List.defaultProps = {
  data: [],
  onSelect: () => {},
}

export default List
