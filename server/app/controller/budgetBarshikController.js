...
async function getAllBudgetBarshik(req, res) {
  const getTotalQuery = `SELECT count(*) as total from budget_barsiks`;
  const getAllBudgetBarshikQuery = `select * from budget_barsiks ORDER BY ? ASC LIMIT ?, ?`;

  pool.query(getTotalQuery, (error, countresults, fields) => {
    if (error) throw error;
    pool.query(
      getAllBudgetBarshikQuery,
      [req.body.budget_barsik_id, req.body.page, req.body.perPage],
...
async function addBudgetBarshik(req, res, next) {
  const addBudgetBarshikQuery = `INSERT INTO budget_barsiks (sirshak_id, karyakram_sirshak_id, dist_id, office_id, user_id, fiscal_year, pratham_chaumasik_amount, doshro_chaumasik_amount, teshro_chaumasik_amount,	barsik_lakshay_amount, created_by, updated_by, createdAt, updatedAt) values (?,?,?,?,?,?,?,?,?, ?,?,?,?,?)`;
  pool.query(
...
async function updateBudgetBarshik(req, res, next) {
  const updateBudgetBarshikQuery = `UPDATE budget_barsiks SET sirshak_id=?, karyakram_sirshak_id=?, dist_id=?, office_id=?, user_id=?, fiscal_year=?, pratham_chaumasik_amount=?, doshro_chaumasik_amount=?, teshro_chaumasik_amount=?,	barsik_lakshay_amount=?, created_by=?, updated_by=?, createdAt=?, updatedAt=? WHERE budget_barsik_id=?`;
  pool.query(
