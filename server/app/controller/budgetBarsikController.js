...
async function getBudgetChaumasikLakshay(req, res) {
  ...
  const getBudgetBarsikChaumasik = `select (${chaumasik_amount} - (select if(isNUll(sum(expense_amount)),0,sum(expense_amount)) as e from budget_karmacharidetails where chaumasik_id = ? and karyakram_sirshak_id = bb.karyakram_sirshak_id  and sirshak_id = bb.sirshak_id and fiscal_year = bb.fiscal_year and office_id=bb.budget_office_id)) as expense_remain,${chaumasik_amount} as chaumasik_amount , (select if(isNUll(sum(expense_amount)),0,sum(expense_amount)) as e from budget_karmacharidetails where chaumasik_id = ? and karyakram_sirshak_id = bb.karyakram_sirshak_id  and sirshak_id = bb.sirshak_id and fiscal_year = bb.fiscal_year and office_id=bb.budget_office_id) expense_amount,(teshro_chaumasik_pariman - (select if(isNUll(sum(expense_pariman)),0,sum(expense_pariman)) as e from budget_karmacharidetails where chaumasik_id = ? and karyakram_sirshak_id = bb.karyakram_sirshak_id  and sirshak_id = bb.sirshak_id and fiscal_year = bb.fiscal_year and office_id=bb.budget_office_id)) as expense__pariman_remain,teshro_chaumasik_pariman as chaumasik_pariman , (select if(isNUll(sum(expense_pariman)),0,sum(expense_pariman)) as e from budget_karmacharidetails where chaumasik_id = ? and karyakram_sirshak_id = bb.karyakram_sirshak_id  and sirshak_id = bb.sirshak_id and fiscal_year = bb.fiscal_year and office_id=bb.budget_office_id) expense_pariman from budget_barsiks bb where bb.sirshak_id like ? and bb.karyakram_sirshak_id like ? and bb.fiscal_year like ? and bb.budget_office_id like ?`
