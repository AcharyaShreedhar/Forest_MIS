...
async function addBanpaidawarLilam(req, res) {
  const addBanpaidawarLilamQuery = `INSERT INTO banpaidawar_lilams (dist_id, lilam_id, lilam_date, banpaidawar_type, unit, quantity, minimum_price, sakaar_price, remarks, created_by, updated_by) values (?,?,?,?,?,?,?,?,?,?,?)`;
  pool.query(
...
async function updateBanpaidawarLilam(req, res) {
  const updateBanpaidawarLilamQuery = `UPDATE banpaidawar_lilams SET dist_id=?, lilam_date=?, banpaidawar_type=?, unit=?, quantity=?, minimum_price=?, sakaar_price=?, remarks=?, created_by=?, updated_by=? WHERE lilam_id=?`;
  pool.query(
