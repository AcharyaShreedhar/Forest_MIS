...
async function getAllBanpaidawarLilam(req, res) {
  ...
  const getTotalQuery =
    `SELECT count(*) as total from banpaidawar_lilams as b where b.lilam_date BETWEEN ? and ? and ${dist_cond} and ${office_cond}`;
  const getAllBanpaidawarLilamQuery = `select * from banpaidawar_lilams as b where b.lilam_date BETWEEN ? and ? and ${dist_cond} and ${office_cond} ORDER BY ? DESC LIMIT ?, ?`;
  pool.query(
    getTotalQuery,
