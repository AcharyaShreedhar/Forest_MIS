...
async function getAllKabuliyatibanBibaran(req, res) {
  ...
  async function getKabuliyatibanBibaran(req, res) {
    ...
    pool.query(getKabuliyatibanBibaranQuery, [req.params.kabuliyatibanBibaranId], (error, results, fields) => {
      if (error) throw error;
      res.send(JSON.stringify({ status: 200, error: null, data: results }));
    });
  }
  ...
  async function updateKabuliyatibanBibaran(req, res) {
    const updateKabuliyatibanBibaranQuery = `UPDATE kabuliyatiban_bibarans SET entry_date=?, ghardhuri_dalit=?, perm_addr=?, curr_addr=?, ghardhuri_janjati=?, ghardhuri_anya=?, ghardhuri_total=?, population_female=?, population_male=?, population_total=?, samudayik_upavokta_samiti_name=?, sampannata_starikaran_sampanna=?, sampannata_starikaran_madhyam=?, sampannata_starikaran_bipanna=?, karyasamiti_representation_dalit=?, karyasamiti_representation_janjati=?, karyasamiti_representation_anya=?, adhyakshya_male=?, adhyakshya_female=?, sachib_male=?, sachib_female=?, created_by=? WHERE kabuliyatiban_bibaran_id=?`;
    pool.query(
