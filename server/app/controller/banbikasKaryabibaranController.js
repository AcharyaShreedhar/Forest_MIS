...
async function addBanbikasKaryabibaran(req, res) {
  const addBanbikasKaryabibaranQuery = `INSERT INTO banbikas_karyabibarans (dist_id, banbikas_karyabibaran, banbikas_ikai, banbikas_parinam, banbikas_bajetkharcha, ban_type, created_by, updated_by) values (?,?,?,?,?,?,?,?)`;
  pool.query(
...
async function updateBanbikasKaryabibaran(req, res) {
  const updateBanbikasKaryabibaranQuery = `UPDATE banbikas_karyabibarans SET dist_id=?, banbikas_karyabibaran=?, banbikas_ikai=?, banbikas_parinam=?, banbikas_bajetkharcha=?, ban_type=?, created_by=?, updated_by=? WHERE banbikas_karyabibaran_id=?`;
  pool.query(
