...
async function getOfficesDropdownList(req, res) {
  const getOfficesList = `SELECT '%' AS office_id, 'सबै' AS value, '' as office_locatoin, '%' As dist_id UNION ALL select office_id as id, office_name as value, office_location, dist_id from offices where dist_id like ? ORDER BY ?`;
...
async function addOffices(req, res) {
  const addOfficesQuery = `INSERT INTO offices (dist_id,office_name,office_location,created_by,updated_by) values (?,?,?,?,?)`;
  pool.query(
    addOfficesQuery,
    [
      req.body.dist_id,
...
async function updateOffices(req, res) {
  const updateOfficesQuery = `UPDATE Offices SET dist_id=?, office_name=?, office_location=?,created_by=?,updated_by=? WHERE office_id=?`;
    pool.query(
    updateOfficesQuery,
    [
      req.body.dist_id,
