...
async function getOfficeStaffPosts(req, res) {
  ...
  pool.query(getOfficeStaffPostsQuery, [req.params.officeStaffPostId], (error, results, fields) => {
    if (error) throw error;
    res.send(JSON.stringify({ status: 200, error: null, data: results }));
  });
}
...
async function addOfficeStaffPosts(req, res) {
  const addOfficeStaffPostsQuery = `INSERT INTO office_staff_posts (post,kayam_darbandi_sankhya,padpurti_sankhya,khali_sankhya,created_by,updated_by) values (?,?,?,?,?,?)`;
  pool.query(
...
async function updateOfficeStaffPosts(req, res) {
  const updateOfficeStaffPostsQuery = `UPDATE office_staff_posts SET post=?,kayam_darbandi_sankhya=?,padpurti_sankhya=?,khali_sankhya=?,created_by=?,updated_by=? WHERE office_staff_post_id=?`;
  pool.query(
...
async function deleteOfficeStaffPosts(req, res) {
