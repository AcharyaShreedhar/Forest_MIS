...
async function getDepartment(req, res) {
  ...
  pool.query(getDepartmentQuery, [req.params.deptId], (error, results, fields) => {
    if (error) throw error;
...
async function addDepartment(req, res) {
  const addDepartmentQuery = `INSERT INTO departments (dept_name_nep,dept_name_eng,created_by,updated_by) values (?,?,?,?)`;
  pool.query(
    addDepartmentQuery,
    [
...
async function updateDepartment(req, res) {
  const updateDepartmentQuery = `UPDATE departments SET dept_name_nep=?, dept_name_nep=?,created_by=?,updated_by=? WHERE dept_id=?`;
  pool.query(
