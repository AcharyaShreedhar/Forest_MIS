...
async function getBanxetraAtikraman(req, res) {
  const getBanxetraAtikramanBibaranQuery =
    "SELECT SUM(IF((Year(atikraman_miti)<=?)&&(atikraman_abastha='0'),atikramit_area,0)) as previousarea,SUM(IF((Year(atikraman_miti)<=?)&&(atikraman_abastha='0'),dalit_ghardhuri+janjati_ghardhuri+anya_ghardhuri,0)) as previousghardhuri,SUM(IF(Year(atikraman_miti)=?,atikramit_area,0)) as currentarea,SUM(IF(Year(atikraman_miti)=?,dalit_ghardhuri+janjati_ghardhuri+anya_ghardhuri,0)) as currentghardhuri,SUM(IF((Year(atikraman_miti)=?)&&(atikraman_abastha='1'),atikramit_area,0)) as hatayekoarea,SUM(IF((Year(atikraman_miti)=?)&&(atikraman_abastha='1'),dalit_ghardhuri+janjati_ghardhuri+anya_ghardhuri,0)) as hatayekoghardhuri,SUM(IF(atikraman_abastha='0',atikramit_area,0))as bakiarea,SUM(IF(atikraman_abastha='0',dalit_ghardhuri+janjati_ghardhuri+anya_ghardhuri,0))as bakighardhuri FROM `banxetra_atikramans` where dist_id like ?";
