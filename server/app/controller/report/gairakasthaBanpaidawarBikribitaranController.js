...
async function getGairakastaBanpaidawarBikribitaran(req, res) {
  const getGairakastaBanpaidawarBikribitaranQuery =
    "SELECT banpaidawar_kisim as name, ekai as unit, SUM(IF(banko_kisim='9',aantarik_parinam+aapurti_parinam+bahiya_parinam,0)) AS sarkarbata, SUM(IF(banpaidawar_kisim='1' || banko_kisim='2',aantarik_parinam+aapurti_parinam+bahiya_parinam,0)) AS samudayik_banbata, SUM(IF(banko_kisim='8',aantarik_parinam+aapurti_parinam+bahiya_parinam,0)) AS niji FROM `banpaidawar_bikribitarans` where dist_id like ? and YEAR(bikri_miti)=? and (banpaidawar_kisim='4'||banpaidawar_kisim='5'||banpaidawar_kisim='6') GROUP BY banpaidawar_kisim";
