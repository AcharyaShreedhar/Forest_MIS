...
async function getRojgariSrijana(req, res) {
  ...
  const getRojgariSrijanaQuery =
    `SELECT karya, ekai, SUM(mahila)as mahila, SUM(purus) as purus, SUM(mahila+purus*ekai) as jamma, kaifiyat as remarks FROM rojgar_srijanas  WHERE ${dist_cond} and ${office_cond} and miti>=? GROUP BY karya`;
