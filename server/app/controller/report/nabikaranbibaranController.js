...
async function getNabikaranBibaran(req, res) {
  ...
  const getKNabikaranBibaranQuery =
    `SELECT SUM(IF(Year(n.renewal_date)<=?,1,0)) AS expired,SUM(IF(Year(n.renewal_date)=?,1,0)) AS renewed,SUM(IF(Year(n.renewal_date)!=?,1,0)) AS remaining,SUM(IF(Year(n.renewal_date)=?,1,0)) as tobeexpired,SUM(IF(Year(n.renewal_date)!=? OR Year(n.renewal_date)=?,1,0)) as torenew FROM kabuliyatiban_bibarans as s LEFT JOIN nabikaran_karyayojanas as n ON s.darta_no=n.darta_id where s.${dist_cond} and s.${office_cond}`;
  pool.query(
